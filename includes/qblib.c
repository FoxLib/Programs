        ...
        while (SDL_PollEvent(& event)) {
            ...
            switch (event.type) {
                ...
                case SDL_KEYDOWN:
                    ...
                    sdl_key_id = get_key_ascii(event);
                    break;

                case SDL_KEYUP:

                    sdl_event_trigger |= EVT_KEY_UP;
                    sdl_key_id = get_key_ascii(event) | 0x80;
                    break;
...
int keycode()  { return sdl_key_id; }
int keypress() { return ((sdl_key_id & 0x80) == 0) && sdl_key_id ? sdl_key_id : 0; }
