...
// Двухмерное преобразование в волновые коэффициенты
void jpeg::DCT8(double v[8][8], double t[8][8]) {
    ...
    for (j = 0; j < 8; j++) {
        ...
        for (x = 0; x < 8; x++) {
            for (y = 0; y < 8; y++) {
                ...
                double k = cos ((2.0*(double)x+1.0)*(double)j*dk) * cos((2.0*(double)y + 1.0)*(double)i*dk);
                sum += k * v[y][x];
            }
        }
...
// Обратное двухмерное преобразование из волновые коэффициентов
void jpeg::IDCT8(double v[8][8], double t[8][8]) {
    ...
    for (x = 0; x < 8; x++) {
        ...
        for (i = 0; i < 8; i++) {
            for (j = 0; j < 8; j++) {
                double k = C(i)*C(j) * cos((2.0*(double)y + 1.0)*(double)i*dk) * cos ((2.0*(double)x+1.0)*(double)j*dk);
                sum += k * t[i][j];
            }
        }
