#define HEAP_STRING_SIZE 65536
#define QB_MAX_WIDTH  640
#define QB_MAX_HEIGHT 480

SDL_Surface*  qb_sdl_screen;

struct mousedata { int x, y, st; };
...
int qb_screen_width,    qb_width,
    qb_screen_height,   qb_height,  qb_font_height,
    qb_screen_mode,
    qb_screen_factor;
int qb_fore_color, qb_back_color;
int qb_locate_x, qb_locate_y;
int qb_autoflip;
int qb_useindex;

// Разрешить скролл по X и Y
int qb_scroll_x, qb_scroll_y;

// Временные строки
char heap_tmpstring[HEAP_STRING_SIZE];
int  heap_location;

// Буфер для бегущей строки
char rec_running_string[32768];
int  rec_ticker_fine;   // 0..7
int  rec_ticker_more;   // 40/80
int  rec_ticker_coarse; // Номер символа

int sdl_event_trigger;
int sdl_trigger_frames;
int rec_frame_id;
mousedata mouse;

uint  qb_palette[256];
int   qb_screen[ QB_MAX_WIDTH ][ QB_MAX_HEIGHT ];
...
enum CUSTOM_EVT {
    ...
    EVT_NONE        = 0,
    EVT_REDRAW      = 1,        // 50 герц обновление экрана
    EVT_IDLE        = 2,
    EVT_MOUSE_MOVE  = 4,
    EVT_MOUSE_DOWN  = 8,
    EVT_MOUSE_UP    = 16,
...
// Шрифт 8x16
static const unsigned char QB_FONT_8x16[256][16] = {
...
static const int QB_DOS_PALETTE_13[256] = {
